...
def generate_python_lexer(header_code, dfa, accept_actions, out_path):
    with open(out_path, 'w', encoding='utf-8') as f:
        ...
        if header_code:
            f.write('# Header del archivo .yal\n')
        ...
        f.write('# DFA\n')
        f.write('DFA_START = %d\n' % dfa['start'])
        f.write('DFA_NUM_STATES = %d\n\n' % dfa['num_states'])
        f.write('DFA_TRANS = ')
        f.write(repr(dfa['trans']))
        f.write('\n\n')
        f.write('DFA_ACCEPTING = ')
        f.write(repr(dfa['accepting']))
        f.write('\n\n')
        f.write('# Actions (copiadas desde el .yal)\n\n')
        f.write('ACTIONS = []\n\n')
        for idx, act in enumerate(accept_actions):
            ...
            if not lines:
                ...
                for L in lines:
                    f.write('    ' + L.rstrip() + '\n')
                f.write('\n')
            f.write(f'ACTIONS.append(_act_{idx})\n\n')
        f.write('def tokenize_text(text, echo=False):\n')
        f.write('    pos = 0\n')
        f.write('    lineno = 1\n')
        f.write('    col = 1\n')
        f.write('    n = len(text)\n')
        f.write('    while pos < n:\n')
        f.write('        cur_state = DFA_START\n')
        f.write('        last_accept = None\n')
        f.write('        last_accept_pos = pos\n')
        f.write('        i = pos\n')
        f.write('        while i < n:\n')
        f.write('            ch = text[i]\n')
        f.write('            trans_map = DFA_TRANS.get(cur_state)\n')
        f.write('            if trans_map is None:\n')
        f.write('                break\n')
        f.write('            # trans_map keys are caracteres directos\n')
        f.write('            to = trans_map.get(ch)\n')
        f.write('            if to is None:\n')
        f.write('                break\n')
        f.write('            cur_state = to\n')
        f.write('            if cur_state in DFA_ACCEPTING:\n')
        f.write('                last_accept = DFA_ACCEPTING[cur_state]\n')
        f.write('                last_accept_pos = i + 1\n')
        f.write('            i += 1\n')
        f.write('        if last_accept is None:\n')
        f.write('            err_char = text[pos]\n')
        f.write('            print(f\"LEX ERROR at line {lineno} col {col}: unexpected {repr(err_char)}\")\n')
        f.write('            pos += 1\n')
        f.write('            if err_char == \"\\n\":\n')
        f.write('                lineno += 1; col = 1\n')
        f.write('            else:\n')
        f.write('                col += 1\n')
        f.write('            continue\n')
        f.write('        lexeme = text[pos:last_accept_pos]\n')
        f.write('        rule_idx = last_accept[0]\n')
        f.write('        try:\n')
        f.write('            res = ACTIONS[rule_idx](lexeme, lineno, col)\n')
        f.write('            if res is not None:\n')
        f.write('                print(res)\n')
        f.write('        except Exception as e:\n')
        f.write('            print(\"Action error:\", e)\n')
        f.write('        for c in lexeme:\n')
        f.write('            if c == \"\\n\": lineno += 1; col = 1\n')
        f.write('            else: col += 1\n')
        f.write('        pos = last_accept_pos\n\n')
        f.write('if __name__ == \"__main__\":\n')
        f.write('    if len(sys.argv) < 2:\n')
        f.write('        print(\"Usage: python lexer.py <inputfile>\")\n')
        f.write('        sys.exit(1)\n')
